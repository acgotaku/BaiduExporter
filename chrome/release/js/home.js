...
function (_Downloader) {
  ...
  function Home() {
    ...
    _ui.default.addMenu(document.querySelectorAll('.g-dropdown-button')[3], 'afterend');
  ...
  _createClass(Home, [{
    ...
    value: function startListen() {
      ...
      window.addEventListener('message', function (event) {
        ...
        if (event.data.type && event.data.type === 'selected') {
          ...
          _this2.start(_core.default.getConfigData('interval'), function (fileDownloadInfo) {
            if (_this2.mode === 'RPC') {
    ...
    value: function getPrefixLength() {
      var path = _core.default.getHashParameter('/all?path') || _core.default.getHashParameter('path');

      var fold = _core.default.getConfigData('fold');

      if (fold === -1 || path === '/') {
        return 1;
      } else if (_core.default.getHashParameter('/search?key')) {
        return 1;
      } else {
        var dir = path.split('/');
        var count = 0;
    ...
    value: function getFiles(files) {
      ...
      for (var key in files) {
        this.fileDownloadInfo.push({
          ...
          link: "".concat(location.protocol, "//pcs.baidu.com/rest/2.0/pcs/file?method=download&app_id=250528&path=").concat(encodeURIComponent(files[key].path)),
          md5: files[key].md5
        });
      }
